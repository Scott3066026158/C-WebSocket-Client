...
bool ToHex(unsigned char src[], int srcLen, char dest[], int destLen)
{
	if (destLen < srcLen * 2 + 1) return false;
	for (int i = 0; i < srcLen; i++)
	{
		char destNum[3] = { 0 };
		if (src[i] < 16)
		{
			sprintf(destNum, "0%x", src[i]);
			strcat(dest, destNum);
		}
		else
		{
			sprintf(destNum, "%x", src[i]);
			strcat(dest, destNum);
		}
	}
	return true;
...
string HMAC_MD5_Hash(const BYTE* inputBuffer, UINT inputCount, const BYTE* userKey, UINT UserKeyLen)
{
	...
	char* result = new char[sizeof(struContext.aState) * 2 + 1] {0};
	ToHex(outputBuffer, sizeof(struContext.aState), result, sizeof(struContext.aState) * 2 + 1);
